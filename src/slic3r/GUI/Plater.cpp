...
void Sidebar::update_presets(Preset::Type preset_type)
{
    ...
    case Preset::TYPE_PRINTER: {
        ...
        auto update_extruder_diameter = [&diameters, &diameter, &nozzle_diameter](int extruder_index, ExtruderGroup& extruder) {
            ...
            int select = -1;
            // ORCA if user defined a custom nozzle in printer config select it instead inherited one. this will show correct nozzle
            // diameter in combobox if its exist in nozzle diameters list
...
void Plater::Calib_Cornering(const Calib_Params& params)
{
    ...
    const std::string cornering_model_path = params.test_model == 0 ? "/calib/input_shaping/ringing_tower.stl" :
                                                                      (params.test_model == 1 ? "/calib/input_shaping/fast_tower_test.stl" :
                                                                                                "/calib/cornering/SCV-V2.stl");
