                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    m_tips_spoolman_ip  = new Label(ip_input_top_panel, _L("Spoolman IP (optional)"));
    m_input_spoolman_ip = new TextInput(ip_input_top_panel, wxEmptyString, wxEmptyString);
    m_input_spoolman_ip->SetMinSize(wxSize(FromDIP(352), FromDIP(28)));
    m_input_spoolman_ip->SetMaxSize(wxSize(FromDIP(352), FromDIP(28)));
    ...
    m_input_top_sizer->Add(0, 0, 0, wxTOP, FromDIP(4));
    m_input_top_sizer->Add(m_tips_spoolman_ip, 0, wxRIGHT | wxEXPAND, FromDIP(18));
    m_input_top_sizer->Add(0, 0, 0, wxTOP, FromDIP(4));
    m_input_top_sizer->Add(m_input_spoolman_ip, 0, wxRIGHT | wxEXPAND, FromDIP(18));
...
void InputIpAddressDialog::set_machine_obj(MachineObject* obj)
{
    ...
    if (auto* app_config = wxGetApp().app_config) {
        m_input_spoolman_ip->GetTextCtrl()->SetLabelText(app_config->get("spoolman_ip", m_obj->get_dev_id()));
    }
...
void InputIpAddressDialog::on_ok(wxMouseEvent& evt)
{
    ...
    std::string str_spoolman_ip = m_input_spoolman_ip->GetTextCtrl()->GetValue().Strip(wxString::both).ToStdString();
    ...
    m_thread = new boost::thread(boost::bind(&InputIpAddressDialog::workerThreadFunc, this, str_ip, str_access_code, str_spoolman_ip, str_sn, str_model_id, str_name));
}
...
void InputIpAddressDialog::on_send_retry()
{
    ...
    wxString spoolman_ip     = m_input_spoolman_ip->GetTextCtrl()->GetValue().Strip(wxString::both);

    // check support function
    if (!m_obj) return;
    if (auto* app_config = wxGetApp().app_config) {
        if (spoolman_ip.IsEmpty()) {
            app_config->erase("spoolman_ip", m_obj->get_dev_id());
        } else {
            app_config->set_str("spoolman_ip", m_obj->get_dev_id(), spoolman_ip.ToStdString());
        }
    }
...
void InputIpAddressDialog::workerThreadFunc(std::string str_ip, std::string str_access_code, std::string spoolman_ip, std::string sn, std::string model_id, std::string name)
{
    ...
    CallAfter([this, detectData, str_ip, str_access_code, spoolman_ip, w]() {
        ...
        if (m_obj) {
            ...
            if (auto* app_config = wxGetApp().app_config) {
                if (spoolman_ip.empty()) {
                    app_config->erase("spoolman_ip", m_obj->get_dev_id());
                } else {
                    app_config->set_str("spoolman_ip", m_obj->get_dev_id(), spoolman_ip);
                }
            }
        }
