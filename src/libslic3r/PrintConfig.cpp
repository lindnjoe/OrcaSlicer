...
void PrintConfigDef::init_common_params()
{
    ...
    def->label   = L("Spoolman hostname, IP or URL");
    def->tooltip = L("Optional hostname, IP address, or URL for Spoolman when it is hosted separately from the printer. "
                     "If empty, Orca Slicer will try the printer host with port 7912.");
    def->mode    = comAdvanced;
    def->cli     = ConfigOptionDef::nocli;
    def->set_default_value(new ConfigOptionString());
