...
void InputIpAddressDialog::on_ok(wxMouseEvent& evt)
{
    ...
    m_thread = new boost::thread(boost::bind(&InputIpAddressDialog::workerThreadFunc, this, str_ip, str_access_code, str_spoolman_ip,
                                             str_sn, str_model_id, str_name));
}
...
void InputIpAddressDialog::workerThreadFunc(
    std::string str_ip, std::string str_access_code, std::string spoolman_ip, std::string sn, std::string model_id, std::string name)
{
